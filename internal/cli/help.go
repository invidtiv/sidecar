...
func RenderHelp(cmd *Command) string {
	...
	if len(cmd.Flags) > 0 {
		buf.WriteString("\nOptions:\n")
		// `shell name` takes no argument-bearing flag, so it keeps the narrow
		// two-column layout. Anything that does — `shell rename --target
		// SESSION` — needs the general renderer, which prints the argument.
		if cmd.Name == "name" {
			for _, f := range cmd.Flags {
				if f.Name == "--json" {
...
func RenderAgents(root *Command) string {
	...
	buf.WriteString("Sidecar commands for agents. sidecar open works from any context; shell name acts on the shell you are running in, and shell rename does too unless you name a --target; shell list, forget, restore, and send act on this project's records; project current/list/add/switch act on Sidecar's configured projects, not on the filesystem.\n\n")
	...
	buf.WriteString("Pane layout is readable and writable: layout get prints the grid, layout apply composes panes onto it, and layout move repositions one pane that is already open.\n")
	buf.WriteString("From a Sidecar-managed pane whose geometry lease is held by a connected viewer, sidecar open and layout land on that viewer's screen; there is no sidecar open --host.\n")
