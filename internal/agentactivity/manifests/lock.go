...
type Lock struct {
	...
	// Files pins the vendored files that are not agent manifests: the upstream
	// LICENSE and the generated NOTICE. They carry the attribution the vendored
	// tree depends on, so they are digest-pinned exactly like a manifest.
...
// LockFile pins one vendored non-manifest file.
...
// File returns the lock entry for a vendored non-manifest file path, such as
// "upstream/LICENSE".
func (l *Lock) File(path string) (LockFile, bool) {
...
// GeneratedNotice is the Origin recorded for upstream/NOTICE, which Sidecar
// writes rather than copies.
const GeneratedNotice = "generated by internal/tools/herdrsync"
