...
func TestNotesWheelUnknownSurfaces(t *testing.T) {
	tests := []struct {
		...
		{name: "inline tmux editor", setup: func(p *Plugin) { p.edit.Active = true }, x: editorX},
		{name: "inline tmux editor over list", setup: func(p *Plugin) { p.edit.Active = true }, x: listX},
