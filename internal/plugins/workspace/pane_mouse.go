...
// capture observed it. Who owns a wheel notch is a property of the pane and is
// asked whether or not the pane holds the keyboard, so the observation has to
// outlive the poll that made it. The key space is the one terminal history and
// pane geometry already use, so a pane has one identity across all three.
...
// producing; a pane no component owns is answered from the last capture that
