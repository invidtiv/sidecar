...
// so the note that survives clipping has to be the one that matters most.
func TestHeaderNotesLeadWithTheWindowBeingOffTheLiveEdge(t *testing.T) {
	...
	if back < 0 {
		t.Fatalf("the header dropped the off-live-edge note:\n%s", view)
	}
	if strings.Contains(view, "app mouse") || strings.Contains(view, "drag select") || strings.Contains(view, "⇧drag") {
		t.Fatalf("the header still states a mouse-reporting note:\n%s", view)
	}
}

func TestWatchedHeaderOmitsAppMouseNotes(t *testing.T) {
	p := newInteractiveInputTestPlugin()
	p.viewMode = ViewModeList
	p.interactiveState = nil
	p.width, p.height = 120, 40
	...
	view := ansi.Strip(p.renderShellOutput(200, 40))
	for _, forbidden := range []string{"app mouse", "drag select", "⇧drag", "⌥wheel"} {
		if strings.Contains(view, forbidden) {
			t.Fatalf("the watched header still contains %q:\n%s", forbidden, view)
		}
	}
}
