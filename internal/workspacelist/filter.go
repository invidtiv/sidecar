...
func (f *Filter) RenderRow(width, matched, total int) string {
	...
	counts := ""
	if f.Active() {
		counts = fmt.Sprintf("%d of %d", matched, total)
	}
	left := prompt + body
	if f.Focused() {
		left = prompt + query + "▌"
	...
	if gap < 1 {
		left = ansi.Truncate(left, max(1, width-ansi.StringWidth(counts)-1), "…")
		gap = max(1, width-ansi.StringWidth(left)-ansi.StringWidth(counts))
	}
	style := styles.Muted
	if f.Focused() {
		style = styles.Title
	}
	row := style.Render(left) + strings.Repeat(" ", gap)
	if counts != "" {
		row += styles.Muted.Render(counts)
	}
	return row
}
