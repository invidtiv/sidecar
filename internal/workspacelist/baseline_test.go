...
// Responsive baseline capture for the shared workspace sidebar.
//
// Project and global Workspaces share one header grammar and one row grammar,
// while still handing RenderSidebar different chrome, section structure, and
// caller-owned row fields. Those legitimate differences are easy to conflate
// with drift because the two surfaces are composed in separate packages and
// never render side by side in production.
//
...
// characterization of the shared grammar; intentional presentation changes
// update it so project/global drift remains visible in one diff.
//
...
func TestSidebarBaselineFixture(t *testing.T) {
	...
	out.WriteString("Workspace sidebar responsive baseline.\n")
