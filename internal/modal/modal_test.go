...
func TestCheckboxSection(t *testing.T) {
	...
	if checked {
		t.Error("Enter on checkbox should not toggle")
	}
	if action != actionSubmitPrimary {
		t.Errorf("checkbox Enter action = %q, want %q", action, actionSubmitPrimary)
	}

	action, _ = s.Update(tea.KeyPressMsg{Code: tea.KeySpace, Text: " "}, "agree")
	if !checked {
		t.Error("expected checked to be true after Space")
	}
	if action != actionOverlayIdle {
		t.Errorf("checkbox Space action = %q, want %q", action, actionOverlayIdle)
	}
...
func TestHandleKeyEsc(t *testing.T) {
	...
	action, _ := m.HandleKey(tea.KeyPressMsg{Code: tea.KeyEscape})
	if action != "cancel" {
...
func TestHandleKeyEnterOnCheckboxSubmitsWithoutToggle(t *testing.T) {
	checked := false
	...
	action, _ := m.HandleKey(tea.KeyPressMsg{Code: tea.KeyEnter})
	if action != "submit" {
		t.Fatalf("Enter on checkbox action = %q, want submit", action)
	}
	if checked {
		t.Fatal("Enter on checkbox should not toggle")
	}
	...
	if !checked {
		t.Fatal("Space on checkbox should toggle")
	}
}
...
func TestHandleMouseHover(t *testing.T) {
	...
	// Hover over button
	m.HandleMouse(tea.MouseMotionMsg{
		X: okRegion.Rect.X,
		Y: okRegion.Rect.Y,
	}, handler)
...
func TestMouseScrollModal(t *testing.T) {
	...
	m.HandleMouse(tea.MouseWheelMsg{
		X:      0,
		Y:      0,
...
func TestListSection(t *testing.T) {
	...
	s.Update(tea.KeyPressMsg{Code: tea.KeyDown}, "list")
	if selectedIdx != 1 {
		t.Errorf("expected selectedIdx 1 after down, got %d", selectedIdx)
	}

	// Test enter returns selected item ID
	action, _ := s.Update(tea.KeyPressMsg{Code: tea.KeyEnter}, "list")
	if action != "item2" {
