...
// LayoutTabStrip is the issue header: an end-truncated title strip so the
// issue ID stays visible. Hosts draw this row and register hit regions from
...
func LayoutTabStrip(group Tabs, width int, focused bool) tabs.Strip {
	return tabs.LayoutStrip(tabLabels(group), group.Active, width, focused, fitIssueLabel)
}
