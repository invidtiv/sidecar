...
func TestOlderHostHelloReadsAsNoVerbCapabilities(t *testing.T) {
	...
	if msg.Hello.Capabilities.Verbs.MobileServeV0 {
		t.Error("a host that never wrote the field was read as supporting mobile serve")
	}
	if msg.Hello.Capabilities.Verbs.MobileOwnerServeV0 {
		t.Error("a host that never wrote the field was read as supporting mobile owner serve")
	}
...
func TestVerbCapabilitiesSurviveTheWire(t *testing.T) {
	...
	if err := encoder.Encode(Message{Kind: KindHello, Hello: &Hello{
		Proto:        Version,
		Capabilities: Capabilities{Verbs: VerbCapabilities{CreateShellAgent: true, ContentReadV1: true, UIRequestRelayV1: true, MobileServeV0: true, MobileOwnerServeV0: true}},
	}}); err != nil {
		t.Fatal(err)
	}
	msg, err := NewDecoder(&buffer).Next()
	...
	if !msg.Hello.Capabilities.Verbs.MobileServeV0 {
		t.Fatalf("MobileServeV0 did not survive the wire: %s", buffer.String())
	}
	if !msg.Hello.Capabilities.Verbs.MobileOwnerServeV0 {
		t.Fatalf("MobileOwnerServeV0 did not survive the wire: %s", buffer.String())
	}
}
