...
"$repo_root/scripts/render-homebrew-formula.sh" \
  v1.2.2 "$sha" "$temporary/current/sidecar.rb" >/dev/null
check_formula_transition \
  "$temporary/current/sidecar.rb" "$temporary/expected/sidecar.rb" v1.2.3 ||
  fail "rejected an upgrade"

cp "$temporary/expected/sidecar.rb" "$temporary/current/sidecar.rb"
set +e
check_formula_transition \
  "$temporary/current/sidecar.rb" "$temporary/expected/sidecar.rb" v1.2.3
status=$?
...
sed -i.bak "s/$sha/1111111111111111111111111111111111111111111111111111111111111111/" \
  "$temporary/current/sidecar.rb"
rm "$temporary/current/sidecar.rb.bak"
...
"$repo_root/scripts/render-homebrew-formula.sh" \
  v1.2.4 "$sha" "$temporary/current/sidecar.rb" >/dev/null
if check_formula_transition \
  "$temporary/current/sidecar.rb" "$temporary/expected/sidecar.rb" v1.2.3 \
  >/dev/null 2>&1; then
...
cp "$temporary/current/sidecar.rb" "$temporary/seed/Formula/sidecar.rb"
git -C "$temporary/seed" add Formula/sidecar.rb
...
# Publisher wants to upgrade to the exact expected formula; racer lands first.
cp "$temporary/expected/sidecar.rb" "$temporary/publisher/Formula/sidecar.rb"
git -C "$temporary/publisher" add Formula/sidecar.rb
...
push_tap_commit "$temporary/publisher" "$temporary/expected/sidecar.rb"
git --git-dir="$temporary/tap.git" show main:Formula/sidecar.rb \
  >"$temporary/remote-formula.rb"
cmp -s "$temporary/remote-formula.rb" "$temporary/expected/sidecar.rb" ||
