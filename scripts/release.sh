...
#   CHANGELOG.md             top heading already stamped `## [vX.Y.Z] - date`
...
if [[ -n $release_version ]]; then
  ...
  needs_stamp=true
elif [[ $top_heading =~ ^'## [v'([0-9]+\.[0-9]+\.[0-9]+)']' ]]; then
...
if $needs_stamp; then
  today=$(date +%Y-%m-%d)
  perl -0pi -e "s/^## \[Unreleased\]/## [$release_version] - $today/m" CHANGELOG.md
  grep -Fq "## [$release_version] - $today" CHANGELOG.md ||
