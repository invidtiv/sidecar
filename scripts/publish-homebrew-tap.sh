...
source_repo=${SIDECAR_SOURCE_REPO:-marcus/sidecar}
tap_repo=${SIDECAR_TAP_REPO:-marcus/homebrew-tap}
release_workflow=${SIDECAR_RELEASE_WORKFLOW:-release.yml}
discovery_timeout=${SIDECAR_RELEASE_DISCOVERY_TIMEOUT:-900}
poll_interval=${SIDECAR_RELEASE_POLL_INTERVAL:-5}
formula_name=sidecar.rb
...
# Return 10 when the exact formula is already published. Distinct from an older
# formula (replace) and a same-version mismatch (investigate, do not overwrite).
...
tap_repo_url() {
  if [[ -n ${SIDECAR_TAP_REPO_URL:-} ]]; then
    echo "$SIDECAR_TAP_REPO_URL"
    return
  fi
...
check_prerequisites() {
  local command_name
  for command_name in curl gh git jq tar; do
  ...
  [[ $discovery_timeout =~ ^[1-9][0-9]*$ ]] ||
    die "SIDECAR_RELEASE_DISCOVERY_TIMEOUT must be a positive integer"
  [[ $poll_interval =~ ^[1-9][0-9]*$ ]] ||
    die "SIDECAR_RELEASE_POLL_INTERVAL must be a positive integer"
...
push_tap_commit() {
  ...
  for attempt in 1 2 3; do
    ...
    if ! cmp -s "$tap_dir/Formula/$formula_name" "$expected_formula"; then
...
# Publish the formula from a rendered file into an already-cloned tap checkout.
# Used by CI (GITHUB_TOKEN checkout) and by the local resume path.
apply_formula_to_tap() {
  local release_version=$1
  local expected_formula=$2
  local tap_dir=$3
  local tap_formula transition_status

  tap_formula="$tap_dir/Formula/$formula_name"
  ...
  git -C "$tap_dir" \
    ...
    commit -m "sidecar: bump to $release_version"
...
publish_tap() (
  ...
  local tap_dir remote_formula remote_version remote_sha remote_commit
  ...
  # Wait until the whole workflow (verify + goreleaser + homebrew) finishes.
  # If the CI homebrew job already published the exact formula, the transition
  # check below is idempotent (status 10).
  ...
  mkdir "$formula_dir"
  expected_formula="$formula_dir/$formula_name"
  ...
  apply_formula_to_tap "$release_version" "$expected_formula" "$tap_dir"

  git -C "$tap_dir" fetch origin main
  remote_commit=$(git -C "$tap_dir" rev-parse FETCH_HEAD)
  remote_formula="$temporary/remote-sidecar.rb"
  git -C "$tap_dir" show "$remote_commit:Formula/$formula_name" >"$remote_formula"
  ...
  echo "published and verified $tap_repo@$remote_commit: sidecar $remote_version ($remote_sha)"
)
