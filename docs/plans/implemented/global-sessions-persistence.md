...
3. **The `PaneLayoutJSON` translation is host code, and live leaves have no codec at all.** The passive half of the codec is already shared: `contentpanes` owns `Encode`/`Decode` over a reference-only `State` — tree, tabs with refs and scroll, and the unknown-kind-collapses degradation rule — and the app-plugin decks persist through it today (`persistAppContentDeck`). What remains on the project plugin is the translation between that world and `PaneLayoutJSON`: `workspaceDeckNode`, `encodePaneNode`/`encodeSurfacePaneLayout`, and the per-kind `decode*Leaf` methods that also serve its legacy non-deck path. And `termpanes` has no persistence shape of its own — a live leaf's session, name, and split placement are encoded nowhere host-independent. The global browser cannot persist without both halves, and duplicating the translation would be the same trap `termpanes` just closed. The refactor precedes the feature, again.
...
2. **The translation moves to one host-independent home before the global surface persists anything.** A new package, `internal/panecodec`, owns `*state.PaneLayoutJSON ↔ (contentpanes.State + live-leaf records)`, building on `contentpanes.Encode`/`Decode` rather than restating its per-kind knowledge, and giving `termpanes` leaves the persistence shape they lack (session name, leaf name, split axis and ratio). Hosts supply what is genuinely theirs — content loaders returned as `tea.Cmd`s, target resolution — through a small options struct, the same division `contentpanes` and `termpanes` already draw. The project workspace adopts it with no behaviour change, proven the same way phase 1 of the extraction was: its suite passes unedited.
...
## Phase 1 — Extract the translation
...
1. **Create `internal/panecodec`** owning both directions of `*state.PaneLayoutJSON ↔ (contentpanes.State + live-leaf records)`. The passive per-kind knowledge stays where it already lives — `contentpanes.Encode`/`Decode` — and the package adds only the `PaneLayoutJSON` projection and the live-leaf shape `termpanes` lacks. Host-owned resolution (project roots, provider registries, issue stores) enters through the options struct.
2. **Adopt it in the project workspace**, deleting `workspaceDeckNode`, `encodePaneNode`, `encodeSurfacePaneLayout`, and `decodePaneNode`. The five per-kind `decode*Leaf` methods retire with the legacy non-deck restore path where they are now only that path's servants; any still load-bearing logic moves into the translation, not into a copy.
