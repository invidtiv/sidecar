...
- `hostproto` stays at `Version = 1` with no request direction, and `hostserve` gains no write paths — so "serve is read-only by construction rather than by flag" remains a property of its call graph, checked by `TestServeIsReadOnly`, rather than a claim that quietly stopped being true. (Superseded after Phase C: serve now has one write, the reap. `TestServeIsReadOnly` still holds the tmux half — serve issues no mutating tmux command — and the state write goes through `shellstate`'s conditional writer. The narrower claim is the true one.)
