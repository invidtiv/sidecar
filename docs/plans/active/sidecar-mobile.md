...
**Status:** readiness, screen study, M0-A/B terminal strategy, M0-C Go service and M0-D native client independently approved and committed. Earlier physical iPhone input, rotation and desktop handoff are observed, along with a physical fresh-process reconnect. Updated-build physical checks and remaining iPad proof keep M0-E open. M1 local and cross-host catalog, exact candidate selection, bounded owner forwarding, native Sessions/filter preferences and frozen History are implemented in independently reviewed slices. The real two-Mac protocol journey passes exact routing, duplicate session/pane identity, retarget refusal, owner loss and no local fallback. Signed native local candidate selection, sustained control, input, History and saved-connection reuse pass after a reviewed responder fix. Native cross-host proof and regular iPad rotation/relaunch checks pass; narrow-to-wide list-position retention still needs a fix and proof; visible software-keyboard swipe and physical retests remain open. Neither M0 nor M1 is declared complete. **Created:** 2026-09-07. **Execution epic:** td-5d82c2. **Plan task:** td-09d533. **Original planning task:** td-c2870b.
