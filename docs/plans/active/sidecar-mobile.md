...
The hub is `aerie`, with a universal native client targeting iOS/iPadOS 26.0 under team `<APPLE_TEAM_ID>` and bundle `com.haplab.sidecar`. The reviewed Go mobile service and cross-host routing are integrated through `e0afd1ae`; the separate host-list reporting correction is integrated at `ba78d35c`. The full Go suite passes at `e0afd1ae`, and the affected full CLI package plus pinned Linux lint pass after that correction. Aerie and MarcusBook run the reviewed `ba78d35c` checkpoint. MarcusBook was built from a pinned detached checkout; its deployed immutable binary, recovery ref, canonical main checkout and prior binary are preserved. The obsolete source checkout has been removed as part of the reviewed worktree cleanup. Normal hub queries reach both owners, and a fresh normal SSH stream opens the exact remote terminal in viewing mode without changing geometry or taking control. Both personal devices have reviewed native `bf174cb`, version `0.1.0` build `5`, installed from one independently verified development-signed Release archive. It includes the History transition and responder fixes, the reviewed downward History gesture, a single-line compact Release control and selected-row restoration across adaptive layout branches. Installation and version checks pass; locked devices prevent a fresh physical interaction claim. Signed 26.5 simulator proof covers local and actual routed MarcusBook terminal selection, sustained input/control, owner History, Release and saved-profile reuse. A later signed gesture run passes hidden-keyboard downward drag, owner History, seven heartbeats and portrait/landscape header checks. The combined signed iPhone/iPad proof now shows the same four local/remote active/inactive rows as desktop and the API, opens both exact owner/session/pane identities in viewing mode, and preserves the remote host filter without changing either terminal’s geometry or owner. The iPad rotation/relaunch baseline proves selection and filters at regular widths. Reviewed native `fba9baa` restores the successfully opened row when adaptive branches or the sidebar return; its labeled forced-size-class proof retains one injected attachment and permits manual scrolling without snapping back. That fix is included in installed build 5. A bounded native-list probe did not establish the stable row identity needed to preserve an arbitrary unselected browsing position, so that M1 implementation gap remains open. The probe changed no shipped code. Actual system window resizing and physical checks remain open. See [execution](sidecar-mobile/execution.md) and the mobile repository's [try guide](../../../../sidecar-mobile/docs/readiness/try-sidecar.md) for exact current checkpoints. Neither M0 nor M1 is declared complete. Concrete Jumar redistribution terms, public-supported-OS hardware proof, app-key enrollment/revocation and final signing/export/tester decisions remain distribution gates; no TestFlight build has been uploaded.
