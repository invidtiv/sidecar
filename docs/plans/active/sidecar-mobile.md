...
**Status:** readiness, screen study, M0-A/B terminal strategy, M0-C Go service and M0-D native client independently approved and committed. Earlier physical iPhone input, rotation and desktop handoff are observed, along with a physical fresh-process reconnect. Updated-build physical checks and remaining iPad proof keep M0-E open. Backend M1-A is independently approved, including local and cross-host catalog, exact candidate selection, bounded owner forwarding and same-config desktop/API all-states parity. Native Sessions/filter preferences and bounded frozen History are implemented in independently reviewed slices. The real two-Mac protocol journey passes exact routing, duplicate session/pane identity, retarget refusal, owner loss and no local fallback. Signed native local candidate selection, sustained control, input, History and saved-connection reuse pass after a reviewed responder fix. Native cross-host proof, same-config four-row desktop/API/iPhone/iPad catalog comparison and exact local/remote viewing journeys pass, along with regular iPad rotation/relaunch checks. The selected-row restoration fix is reviewed, with forced compact/regular and sidebar transitions preserving one injected attachment; arbitrary unselected browsing-position retention remains an implementation gap, while actual window resizing, visible software-keyboard swipe and physical retests remain open. Neither M0 nor M1 is declared complete. **Created:** 2026-09-07. **Execution epic:** td-5d82c2. **Plan task:** td-09d533. **Original planning task:** td-c2870b.

This is the controlling product and architecture plan. Read [M0/M1 execution](sidecar-mobile/execution.md) next for exact source seams, ownership, task dependencies, readiness gates, and the terminal-seed decision. [Touch scrolling over a controlled terminal](sidecar-mobile/touch-scroll.md) covers one-finger scrolling of agent harnesses that own their own scrollback; its three code slices are implemented and reviewed in the mobile repo, and its device proof is open. Implementation is delegated to sub-agents; the coordinator integrates independently reviewed slices. Tooling and screen artifacts live in `../sidecar-mobile`; their existence does not prove terminal attachment.
...
- Offer Activity, Project, Recent, and Name sorting, matching Sessions semantics, plus host/provider/state filters and search. Hide idle sessions by default using the same shared rule as desktop Sessions with Show idle sessions unchecked. Provide Show idle sessions in Filters and persist an explicit mobile choice; existing preferences that predate this toggle adopt the hidden default. Persist these view preferences on the phone; do not change the desktop's sort order. Extract any Go sort semantics needed by the API rather than maintaining a second activity-ranking rule in Swift.
...
- Tap a live shell to open its terminal. A worktree with several possible panes offers a picker; an ambiguous or non-live target never silently attaches to whichever pane happens to share a name. With Show idle sessions enabled, a worktree with no terminal remains inspectable and says no terminal is running. Starting sessions is outside the initial scope.
- Keep background-refresh progress in the existing session/host counts row. Refresh must not insert a separate row below search or shift the visible list. Keep the last catalog visible while requesting its replacement.
...
Keep the current terminal visible during routine control and keyboard geometry refreshes. Show compact progress in the terminal header; do not replace the entire screen with a loading overlay on each transition. Initial attachment without a frame and a sustained stalled refresh may show a more explicit loading explanation. Retaining the image never authorizes input: existing ownership, accepted-geometry and current-frame checks still gate interaction, and a different target or attachment must never inherit the old terminal's contents.
...
Headless tmux clients must preserve UTF-8 machine output without depending on a login shell, a PTY, or inherited `LANG`/`LC_*` settings. Prove catalog discovery, exact target opening and bounded History with those locale variables absent. A simple pane-count command is insufficient: test the actual formatted output consumed by the service, including its separators.
...
The hub is `aerie`, with a universal native client targeting iOS/iPadOS 26.0 under team `<APPLE_TEAM_ID>` and bundle `com.haplab.sidecar`. Aerie now runs reviewed Go `bfed83fb`. Catalog fix `9ab5d62e` removed repeated target scans that exceeded the hub's eight-second owner timeout. The subsequent physical build 6 report exposed a second issue hidden by the shell proof's inherited UTF-8 locale: tmux replaced tab separators when the phone's SSH process had no locale settings, so the inventory discarded all local panes. The shared client argument helper now explicitly requests UTF-8 for discovery, target inspection, control-mode attachment, captures, cursor and lease reads. Malformed inventory output returns an error instead of becoming an empty local inventory. Under absent locale settings, root's side-by-side owner-only stdio proof returned zero rows from the old build and all 17 ready Aerie rows from the corrected candidate in 1.058 seconds. The final installed check returns 17 local plus one MarcusBook ready row, both hosts online, with no failures or terminal actions. Focused race tests, the full Go suite, build and pinned Linux lint pass. One full-suite measurement initially failed because its process lookup expected the old tmux argument string; the reviewed test correction recognizes the same parent's control client with the new UTF-8 flag. The full suite then passes. The shared `show_idle_sessions` query filters only the final No Session group; native explicitly sends false by default, while omission preserves legacy all-row behavior. See the [Aerie catalog proof](sidecar-mobile/proof/aerie-catalog-live-fix.md).

MarcusBook retains its compatible reviewed `ba78d35c` deployment. Its immutable binary, recovery ref, canonical main checkout and prior binary are preserved. Fourteen unused mobile worktrees have been removed through reviewed, identity-pinned cleanup; three remain because running processes still hold them. The cleanup changed neither the default tmux server nor the original private live-agent environment.

Both personal devices have reviewed native `995214a`, version `0.1.0` build `7`, installed from one independently verified development-signed Release archive. Installation and installed-version checks pass on both devices; both refused their single normal launch because they were locked. App data and Keychain credentials were preserved. The build includes reviewed `b698431`: idle sessions hidden by default with a saved per-hub toggle, progress in the existing catalog header, and retained matching terminal content during ordinary refresh. Exact target/attachment identity and accepted-frame input gates remain intact. Independent review corrected replacement-attachment Back navigation and stopped progress on failed refresh. Focused evidence includes 31 client tests, 35 app tests, four signed iPhone journeys and one signed iPad retained-terminal journey. These use injected responses and are distinct from live SSH proof. This slice has portrait phone/iPad runtime evidence; two landscape attempts left the window portrait and are excluded from positive evidence. Reviewed `d23b1a2` separates Host status, Unavailable projects and Catalog issues by the server's failure scope; its focused unit and signed phone UI check both pass. Packaging verifies the actual universal bundle, development signature, exact privacy/notices resources, all 220 source hashes and absence of all 11 DEBUG routes. Reconnecting from a newly opened app uses the corrected hub. The final physical catalog and interaction checks remain pending.

Earlier signed 26.5 simulator proof separately covers local and actual routed MarcusBook terminal selection, sustained input/control, owner History, Release and saved-profile reuse. A signed gesture run covers hidden-keyboard downward drag, owner History, seven heartbeats and portrait/landscape header checks. The combined signed iPhone/iPad proof compares four local/remote active/inactive rows with desktop and the API and opens exact owner/session/pane identities in viewing mode without changing terminal geometry or control. Reviewed selected-row restoration remains in the app; its forced-size-class proof retains one injected attachment and permits manual scrolling without snapping back. A bounded native-list probe did not establish the stable row identity needed to preserve an arbitrary unselected browsing position, so that M1 implementation gap remains open. Actual system window resizing and physical checks remain open.

See [execution](sidecar-mobile/execution.md) and the mobile repository's [try guide](../../../../sidecar-mobile/docs/readiness/try-sidecar.md) for proof boundaries and current checkpoints. Neither M0 nor M1 is declared complete. Concrete Jumar redistribution terms, public-supported-OS hardware proof, app-key enrollment/revocation and final signing/export/tester decisions remain distribution gates; no TestFlight build has been uploaded.
