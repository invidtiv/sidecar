...
**Status:** active; Phase A and Phase B implemented on 2026-08-30 (Phase B's real-binary exit-gate run outstanding), Phase C onward planned. M6 of [Notification sounds and native delivery](notification-sounds-and-native-delivery.md) **Tracking:** `td-43a93f` (Phase B: `td-b5a467`) **Created:** 2026-08-30 **Updated:** 2026-08-30 **Evidence baseline:** Sidecar `main` at `48ddd720`; OpenCode 1.18.23 and 1.18.25, Codex 0.151.0, Claude Code 2.1.220, Pi 0.84.3 surveyed, OpenCode traced including cancellation. Herdr `4a3b04f59ba3b7d8a15cea187b23e1e80c343b0c` inspected for comparison.
...
**Phase B implemented 2026-08-30 (`td-b5a467`), except the real-binary exit-gate run.** Steps 1–5 are built and tested; the exit gate is satisfied in code against the real store, resolver, tracker, and `notify.LaneTracker`, but has **not** been run with the real bundled asset inside a live isolated Sidecar. That run is the one outstanding item and is recorded honestly below rather than claimed.
...
**One design correction worth carrying forward.** The managed shell publishes the tmux server **PID** as the incarnation namespace, not `tmuxserver.Incarnation.String()`. That string embeds the socket ctime, which tmux bumps every time the attached-client set changes, so namespacing stored records by it would have orphaned every report the moment a user attached or detached a client — silently returning a healthy pane to screen fallback. The PID is stable for the server's lifetime and new after a restart, which is exactly the property the namespace needs, and it is what `agentcontrol.sameOccupant` already compares.
...
**Phase B honest limits.** The exit gate has not been run against a live OpenCode inside an isolated Sidecar with the real bundled asset installed; the one link not exercised anywhere is the subprocess boundary between the asset and `sidecar agent report`, which is covered only from the CLI side. `sidecar agent explain --shell TARGET` refuses rather than answering, because explaining another shell needs that shell's pane and run identity from the managed-shell inventory. Integration status is inferred from the recorded asset version rather than from an installed-file inspection, which is Phase C's job. There is no installer: the asset must be placed by hand until Phase C, which also means no user can accidentally enable this yet.
