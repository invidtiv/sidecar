...
**Status:** Planned — M6 of [Notification sounds and native delivery](notification-sounds-and-native-delivery.md) **Tracking:** `td-43a93f` **Created:** 2026-08-30 **Updated:** 2026-08-30
...
The first implementation steel thread is one provider whose public integration API and recorded fixtures cover prompt submission, tool work, permission/question blocking, completion, cancellation, session change, and process exit. OpenCode is the leading candidate because its plugin event model and Herdr's existing integration demonstrate that complete shape, but the contract spike must confirm the current released API before implementation begins. Codex and Claude follow through the same core only after their current hook events are traced across the same cases. No provider receives full lifecycle authority based on documentation or a happy-path test alone.
...
| --- | --- | --- |
| OpenCode | First full-lifecycle candidate and end-to-end steel thread. | Real traces cover work, tool use, permission/question blocking and resolution, completion, cancellation, child sessions, restart, and process exit. |
