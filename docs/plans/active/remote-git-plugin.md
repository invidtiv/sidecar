...
- **A bound "open in GitHub" refuses at its door rather than resolving a remote.** `repo status` returns the host's remote URL, but nothing carries it to `openCommitInGitHub` yet, and calling `GetRemoteURL` with a bound pane's empty root would run git in whatever directory Sidecar was started from. It returns nothing and is not advertised; the working link belongs with the refusal table in 4j, which is where the contract row for it lives.
...
- **The mouse stays inert while bound.** `updateRemote` routes no `tea.MouseMsg`, as it has since 4g, so clicking the commit list does nothing. That is a gap to close deliberately with a mapped subset, not by handing the local mouse handler a pane whose rows reach writes.
