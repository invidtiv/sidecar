...
**Status:** Phases 0, 1, and 2 implemented 2026-09-01; Phase 3 (automation and local overrides) is next. Phase 0: vendored tree with lock, regex compatibility and read window recorded in [the reference document](../../reference/herdr-detection-parity.md), alias parity fixed. Phase 1: engine, 36 ported conformance tests, `explain --file`, differential harness at zero disagreements against herdr 0.8.2. Phase 2: all ten providers detect through the manifests; the Go rule tables, `Evaluate`, and the shadow flag are deleted; eight overlays carry twenty-two Sidecar rules, each with a fixture; the live matrix passes for the six providers installed on the maintainer's machine (Muse now correctly refuses on an unanswered trust prompt it used to read as idle; Pi's failure predates the cutover, since its upstream manifest has no idle rule). **Research baseline:** Sidecar `main` at the head of `claude/tui-lifecycle-herdr-parity-cu2d9t`; Herdr `e2b85c7` (`preview-2026-08-31`, ten commits past v0.8.2), the live catalog at `https://herdr.dev/agent-detection/index.toml`, and Herdr's Apache-2.0 `LICENSE`.
...
The hooks-lane half of parity. Herdr has 17 installable integrations; Sidecar has three (OpenCode as a plugin, Codex and Claude as hook installers). This phase makes porting the remaining ones, and re-porting when upstream bumps, a mechanical job with a fixed shape, so the maintenance cost is a diff review per bump rather than a fresh investigation.

**Why porting is mechanical.** Every Herdr asset, whatever its language (`sh`, `ps1`, `ts`, `js`, `py`), has the same two halves:

- A *provider half*: which hook or plugin event maps to `working`, `blocked`, `idle`, or a session reference; the ordering guards (`seq`, settled-event debouncing, release on exit); the per-provider quirks (Codex ignores hooks whose `session_id` disagrees with `CODEX_THREAD_ID`; Claude drops subagent-completion hooks so they cannot revive an idle pane). This is the knowledge, and it is kept verbatim.
- A *transport half*: gate on `HERDR_ENV=1`, `HERDR_PANE_ID`, `HERDR_SOCKET_PATH`; write one JSON-RPC line (`pane.report_agent`, `pane.report_agent_session`, `pane.release_agent`) to the socket. This is swapped, one to one, for Sidecar's: gate on `SIDECAR_MANAGED_SHELL=1` and `SIDECAR_BIN`; spawn `$SIDECAR_BIN agent report --kind <provider> --state <state> --seq <n>` (or `report-session`, or `release`). The existing OpenCode plugin at `internal/agentintegration/assets/opencode/sidecar-lifecycle.js` is the reference translation; its spawn-serialisation and timeout code becomes a shared helper each ported asset includes rather than re-implements.

**Shape of a port.** Per provider:

1. The vendored upstream asset is already present from the sync (Phase 3, step 5). The Sidecar asset lives beside the existing ones under `internal/agentintegration/assets/<provider>/`, with a header recording `ported-from: herdr <provider> integration version N at <commit>` so the sync report can compute the upstream diff since the port.
2. An `agentintegration.Adapter` implementing install, status, update, repair, uninstall for that provider's config location, following the three that exist. Herdr's installers (`targets.rs`, `config_edit.rs`, `claude_settings.rs`, `opencode_config.rs`) are the reference for *where* each provider reads its hooks and *what* a safe edit looks like (Herdr preserves key order and formatting in Claude settings, refuses extension-directory collisions for Pi and OMP, respects `PI_CODING_AGENT_DIR` and `PI_CONFIG_DIR`); the Go adapter reproduces those rules under Sidecar's ownership contract (only Sidecar-owned changes are ever removed).
3. A capability entry in `capabilities.json` starting at the tier the lifecycle plan's rules allow before traces, then traced against a released provider version the same way OpenCode was, and promoted only on evidence. Herdr's own authority table (`authority.upstream.json`) says which tier is *achievable*: `full` for Pi, OMP, Kimi, Kilo, MastraCode; session-identity for the rest.
4. Fixtures: the hook payloads Herdr's shared `herdr-agent-state.test.ts` exercises are the starting corpus for the Go `hooktrace` tests, translated to Sidecar's report shape.

**Order.** Providers where Herdr holds lifecycle authority first, since those are the ones where the screen lane cannot reach parity: Pi, Kimi, Kilo, MastraCode, OMP. Then session-identity ports in order of live use, which mostly means confirming that Sidecar's existing Codex and Claude adapters cover what Herdr's do (Herdr's Claude asset is at version 9 and Codex at 8; the sync report will show the diff against whatever version Sidecar's adapters were written against).

**Maintenance.** The sync report is the whole loop: an upstream version bump for a ported provider shows the diff since the port, the maintainer decides whether the provider half changed (re-port) or only the transport or packaging half (ignore), updates the header, and the trace tests either still pass or show which transition moved. This is more upkeep than the manifest lane, and it is accepted on purpose: owning the assets is what keeps Sidecar's env vocabulary, installer ownership rules, and evidence tiers intact.

**Exit gate:** every provider in Herdr's lifecycle-authority list has a Sidecar adapter and asset; `TestHerdrAuthorityGaps` prints an empty list or only providers Herdr added since the last sync.
...
6. **The differential harness uses Herdr's release binary**, and the sync pins vendored manifests to the matching release tag by default. The harness section records why a binary is needed at all. **Amended 2026-09-01:** "release tag" means the newest release of any kind, preview builds included, not the newest stable one. Herdr cuts a preview roughly weekly and ships the same `herdr-linux-x86_64` asset on it, so a preview satisfies this decision's whole reason, which is that the vendored files match the binary the harness downloads. Restricting it to stable releases would have been a regression rather than a pin: the newest stable tag is `v0.8.2`, sixty-one commits behind the vendored `e2b85c7`, and `muse.toml` does not exist there at all, so a default sync would have deleted a manifest Sidecar detects through.
