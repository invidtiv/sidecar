...
**Follow-on:** the restore work that remains after M3 and M4, including typed resume commands under the `ask` policy, candidates for shells whose conversation was never reported, worktree sessions, and a stuck-server diagnosis, is implemented in [Session restore: resume the conversation, not only the shell](../implemented/session-restore-resume-conversations.md).
