...
**Codex's third mutation belongs to Codex.** Codex records a per-hook `trusted_hash` in `config.toml`. The algorithm is unpublished and could not be reproduced from the recorded pair on this machine (sha256 over the command, the state key, their concatenations, the hook object in several JSON encodings, and the whole file all missed), so rather than write a guessed hash the proof installed `hooks.json` plus `[features] hooks = true` with no trust entry and observed what Codex does: it presents an interactive "Hooks need review" prompt, and once answered **Codex writes the hash itself**. The adapter therefore owns two mutations and documents that the next launch asks for a one-time trust; inventing the third would be guessing at a security control.
