...
| Not building | Why |
|---|---|
| **A pane tree in `internal/overview`** | `termpreview`'s package doc records the deliberate decision (verified, `termpreview.go:4-19`): *"Global Workspaces does not instantiate a pane tree at all: its right side is one terminal box, and it gets that box from the same functions the plugin's leaf box is built from."* Forcing a tree there is the "second independent geometry computation" that doc names as the biggest failure mode. **Conflict resolved: C wins over A's P4.** The tree, floors, and neighbour math must live where overview *could* call them — that's what makes A's eventual adoption cheap if it's ever wanted — but proving parity is not a reason to build it. |
...
6. **Does the global overview ever get splits?** Recommendation is no, and `termpreview`'s package doc treats that as settled. Confirm it stays settled, or reopen it — the answer changes whether M3's extraction targets one consumer or two.
