...
1. **Sidecar claims a `session-identity` tier for `pi` from a source with no installer.** `capabilities.json` carries `pi` at `session-identity` with source `sidecar.pi.extension`, and `internal/agentsession/trust.go` knows that source id, but there is no `PiAdapter` and no asset under `internal/agentintegration/assets/`. Either an asset exists somewhere this survey missed, or the entry was recorded ahead of the port. Resolve it first: if the tier is unearned it must come out, because the whole capability matrix's value is that it never claims more than it has proved.
...
Answer open question 1 first and act on it: if the `pi` tier is unearned, remove it and let `TestHerdrAuthorityGaps` show `pi sidecar=(none)` until a port earns it back.
