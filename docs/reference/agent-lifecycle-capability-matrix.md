...
- **The blocked lane is unreachable, confirmed from the emitter side.** A `bash` tool ran unconditionally with no permission, approval or prompt event anywhere around it, and nothing published either `sidecar:blocked` or `herdr:blocked` across the run. The docs-only reading of the type definitions was right.
