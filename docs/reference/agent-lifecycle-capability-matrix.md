...
Pi's row went out of `capabilities.json` when nothing could produce a report for it and came back when something could. It is back at `session-identity` on `docs-only` evidence: `PiAdapter` and `assets/pi/sidecar-lifecycle.js` now ship, so the source can speak, but **no live Pi session has been traced**, so the entry claims only `session_identity` and the port's own known gaps. `work_start` and `turn_complete` are what the asset reports and what a trace would earn; `advisory` is the ceiling. See "Why the Pi entry was retracted, and what brought it back".
...
That port has now landed. `PiAdapter` installs `sidecar-lifecycle.js` into `$PI_CODING_AGENT_DIR/extensions` (`~/.pi/agent/extensions` by default), the asset's provider half is Herdr's version 8 mapping kept verbatim in behavior, and the entry is back at `session-identity` with `evidence: docs-only` — the honest floor for a source with an installer and no traces. `TestHerdrAuthorityGaps` now prints `pi ... sidecar=session-identity` rather than `sidecar=(none)`, which is still below Herdr's target and correctly so.

What the entry claims is deliberately narrow. `session_identity` is covered because the asset binds Pi's own session file on every `session_start` and every `agent_start`. `tool_use` is not, because the asset subscribes to no tool event. `process_exit` is not, because the asset does not subscribe to `session_shutdown` at all: three of its five reasons are a session swap rather than an exit, and releasing the lane on those would hand a live pane back to screen detection mid-run. `blocked_on_request` and `unblocked` are not, and never will be. Promoting `work_start` and `turn_complete` — and with them the tier — is trace work, and the entry's second known gap says so in as many words so the next reader knows which fields move together.
