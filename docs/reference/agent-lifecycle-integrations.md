...
- **Replacements are recoverable.** Whenever Sidecar replaces or rewrites a file, it first copies what was there to `<name>.sidecar-backup` beside it. That covers `install`, `update`, and `repair` for every adapter, and it covers `uninstall` for the adapters that rewrite a user-owned file rather than delete their own — Codex's `hooks.json` and `config.toml` and Claude's `settings.json` are backed up before Sidecar's entry is removed from them, and those backups are left in place for you to inspect or discard. OpenCode is the one that leaves nothing behind: its asset is a whole file Sidecar owns, so uninstalling removes the asset and the backup, and a "clean" tree has no Sidecar file left in it.
- **`uninstall` discards local edits; `repair` preserves them.** These are different verbs on purpose. Repair backs up a modified Sidecar-owned asset and writes the bundled one over it, so your edits survive in `<name>.sidecar-backup`. Uninstall is a request to remove Sidecar's file, and it removes it — a hand-edited OpenCode asset is deleted with no backup kept, because keeping one would leave a Sidecar file in a tree the user just asked to be rid of. If you have edited an asset and want the edits, copy them out before uninstalling, or run `repair` instead.
- **Uninstall removes only what Sidecar installed.** The asset, a duplicate copy Sidecar owns, and — for a file-owning adapter — the backup. Entry-owning adapters remove their entry and leave every other byte of the user's configuration file alone. The provider's own configuration and every unrelated plugin are untouched. A plugin directory is removed only when removing Sidecar's files leaves it empty.
...
Every value in the table is derived at the moment the shell is created, and any value Sidecar cannot determine then is omitted rather than invented. The one this happens to in practice is `SIDECAR_TMUX_SERVER`: the first shell created on a socket is created before there is a tmux server to have a PID, so it carries no claimed server. A hook must therefore treat an absent value as *unverified*, not as a mismatch — which is what Sidecar itself does, guarding the comparison on a non-empty claimed server and falling back to the live server PID when there is none.
