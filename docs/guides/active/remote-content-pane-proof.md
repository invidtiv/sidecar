...
This is the two-machine (or loopback) recipe for [remote host content-pane parity](../../plans/active/remote-host-content-pane-parity.md) slice 7. It proves that a click in a remote Sessions terminal loads that host's file, issue, note, diff, resource, or tmux session, and never this machine's same-named twin.
