...
The endpoint exposes only fixed numeric counts plus `output_to_frame_samples`, `output_to_frame_p95_us`, and `output_to_frame_max_us`; it contains no terminal text, paths, targets, or session names. In addition to model, terminal, row, link, and global Sessions counters, it reports application/Workspace/sidebar views, project preview composes/cache hits, document frame builds/cache hits, document scans/resolution work, and row-analyzer bypasses. Snapshot before and after a settled phase and subtract monotonic counts. A pulse-only interval should show preview cache hits with zero preview composes, terminal views, document builds/scans, row misses, or analyzer bypasses. Model-frame counts are process-wide aggregates across every live pane-model feed, so they prove a per-feed frame-rate cap only when the harness independently guarantees one feed. Use the deterministic single-feed cadence tests for the 30 fps contract otherwise. Always stop the explicit run root on success or error.
