...
- **Every agent's state detection now runs Herdr's published detection manifests instead of Sidecar's hand-written rules.** All ten providers Sidecar identifies — Claude Code, Codex, OpenCode, Cursor, Grok, Pi, Copilot, Amp, Antigravity and Muse — are classified by the vendored manifests, so an upstream rule fix arrives without anyone translating a regex. What that fixes today, beyond the Claude and Codex entries below: a Muse pane sitting on an unanswered "Do you trust this workspace?" prompt reads as blocked instead of as a finished turn; a Copilot pane waiting for background agents reads as working; Cursor's approval prompts are recognised by the control lines every one of them renders rather than by a regex per prompt shape, so a prompt Cursor adds next release is already covered; and a Claude pane on an "Allow Claude to use …?" permission prompt, or a Codex pane on an approval prompt whose option line carries the composer glyph, no longer read as idle. Sidecar's own findings are kept as data overlays with a fixture each, and the pane's status is still gated by Sidecar's stricter process check before any rule is evaluated.

- **Claude Code and Codex state detection runs Herdr's published detection manifests instead of Sidecar's hand-written rules.** Claude's half-circle busy spinner (2.1.228 and newer) is recognised, so a working pane no longer has to be inferred from the absence of an idle prompt; MCP elicitation dialogs, the `/btw` overlay, "Waiting for N background agents" and "N MCP tasks still running" are recognised as work in progress; and Codex's first-run "Do you trust the contents of this directory?" prompt now reads as blocked instead of idle. The two things Sidecar knows that upstream does not are kept as data overlays: Claude's model picker still holds the prior state instead of reading as a finished turn, and a Codex turn parked on a background terminal still reads as working. Future upstream rule fixes arrive without anyone translating a regex.
