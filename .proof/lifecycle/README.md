...
| `L2b-after-close-click` | an SGR click on that `×` closes the split: the primary is back to 115x45 full width, the `◧◨` badge is gone, and the split's tmux session `sidecar-tp-…` is closed with it (the confirm is skipped because only the login shell was running). NOTE: this capture predates the fix that made an explicit close end the session; at capture time the session survived the click. |
